...
![img](windows/4.1.1start-setting.png)
...
- 设置模版：windows版本中没有找到设置的地方，好吧，我们跳过这一步
...
![img](windows/4.2.1clone-git.png)

添加成功后，自动进入仓库

![img](windows/4.2.1clone-git-ok.png)

打开新标签后，会发现所有仓库列表多了个仓库

![img](windows/4.2.1clone-git-list.png)
...
![img](windows/4.2.2add-local-git.png)
...
![img](windows/4.2.3create-local-git.png)
...
![img](windows/git-flow/4.3.1git-flow-detail.png)


#### 4.3.2 建立新的功能

![img](windows/git-flow/4.3.2git-flow-feature.png)

功能开发完毕后，在feature分支点击Git工作流后，出现如下提示

![img](windows/git-flow/4.3.2git-flow-feature-end.png)

点击完成当前项目，出现提示，默认会在完成功能后删除`feature`分支，会自动进行分支合并

![img](windows/git-flow/4.3.2git-flow-feature-check.png)

#### 4.3.3 建立新的发布版本

> 默认从develop分支新建

![img](windows/git-flow/4.3.3git-flow-release.png)

在release分支点击Git工作流后，出现如下提示

![img](windows/git-flow/4.3.3git-flow-release-end.png)

点击完成当前项目，会出现如下信息，需要给发布的版本打`tag`同时默认删除`release`分支

![img](windows/git-flow/4.3.3git-flow-release-check.png)
...
![img](windows/git-flow/4.3.4git-flow-hotfix.png)

bug修复完毕后，在hotfix分支点击Git工作流后，出现如下提示

![img](windows/git-flow/4.3.4git-flow-hotfix-end.png)

点击完成当前项目，会出现如下信息，需要给发布的补丁打`tag`同时默认删除`hotfix`分支

![img](windows/git-flow/4.3.4git-flow-hotfix-check.png)

最后仓库的状态

![img](windows/git-flow/4.3.4git-flow-all.png)

标准模式的Git flow会自动进行分支的新建删除，整个过程如下

![img](windows/git-flow/4.3git-flow-all.png)
...
![img](windows/4.4.1commit-code.png)
...
![img](windows/4.4.2stash-apply.png)
...
![img](windows/4.4.3cherry-pick.png)
